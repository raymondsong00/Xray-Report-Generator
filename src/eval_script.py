...
def plot_sim_scores(test_inference, col='similarity', save_path=None):
    ...
    sns.set_theme(style="whitegrid")  # Set the seaborn style for more appealing plots
    ...
    ax.set_title(f'Generic Prompt {col.capitalize()} Scores', fontsize=18)
    ax.set_xlabel(f'{col.capitalize()} Score (0-1)', fontsize=16)
...
def sim_score_by_author(test_inference, author, sim_col='similarity', save_dir=None):
    '''creates a histogram of that author's sim scores'''
    fig = plt.figure(figsize=(10, 8))
    filtered = test_inference[test_inference['author'].str.contains(author)]
...
def plot_author_medians(test_inference, sim_col='similarity', k=10, save_dir=None):
...
def main(output_path, test_inference_path, train_data_path, test_inference_radiologist_answers_path):
    ...
    sim_score_by_author(test_inference, 'Hsiao', save_dir=plot_path)
    sim_score_by_author(test_inference, 'Hsiao', sim_col='findings_similarity', save_dir=plot_path)
    sim_score_by_author(test_inference, 'Hsiao', sim_col='impression_similarity', save_dir=plot_path)
    
    
    # plot author similarity medians (kwarg k for top k authors to plot)
    _, top_k_authors = plot_author_medians(test_inference, save_dir=plot_path)
    
    logging.warning(top_k_authors)
    for author in top_k_authors:
        sim_score_by_author(test_inference, author, save_dir=plot_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_inference_answers_path", help="path to csv with ground truth answers for test inference data")

    
    args = parser.parse_args()
    
    main(args.output_path, args.test_inference_path, args.train_data_path, args.test_inference_answers_path)
